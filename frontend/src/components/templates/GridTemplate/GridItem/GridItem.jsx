  ...
  return (
    <Link to={`/projects/${projectId}`} className="grid-item shadow-sm">
      <div className="flex flex-col gap-1 projects-title-des-wrapper">
        <p
          className={`text-xl text-[var(--dt-title-text)] whitespace-nowrap font-semibold project-title`}
        >
          {` • ${projectName} `}
        </p>
        <p className="line-clamp-2 tracking-[var(--other-text-letter-spacing)] text-[var(--dt-subtitle-text)] project-description">
          {projectDescription ||
      ...
      <div className="flex gap-[6px] flex-wrap text-xs mb-3 leading-3">
        <span className="text-sm text-[var(--dt-title-text)] font-semibold">
          Tags:
        </span>
